...
import { resetPasswordRequest } from "../../api/requestAuth";
...
export default function ResetPasswordForm() {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setSuccess(true);
            // Send them to login after a short pause so they see the
            // success state before the redirect.
            setTimeout(() => navigate("/login"), 2500);
        } catch (err) {
            // Backend returns 401 with "invalid or expired reset token"
            // for a used/expired/malformed token — surface that message
            // directly since it tells the user what to do next (request
            // a new link).
