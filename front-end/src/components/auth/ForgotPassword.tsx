...
import { forgotPasswordRequest } from "../../api/requestAuth";

export default function ForgotPasswordForm() {
    const [email, setEmail] = useState<string>("");
    const [error, setError] = useState<string | undefined>(undefined);
    const [isSubmitting, setIsSubmitting] = useState<boolean>(false);
    // Once true, we show the generic "check your email" message and stay
    // there — matching the backend's design of never revealing whether
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            // Always show success, even on backend error — the request
            // itself should already be failing closed on the server side
            // (returns 200 either way), but this is a defensive fallback
            // so the UI never leaks account existence via a different path.
            setSubmitted(true);
        } catch (err) {
